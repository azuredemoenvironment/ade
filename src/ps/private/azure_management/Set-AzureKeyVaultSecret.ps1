function Set-AzureKeyVaultSecret {
    ...
    az keyvault secret set -n $secretName --vault-name $keyVaultName --value $secretPlainTextValue --only-show-errors
    Confirm-LastExitCode

    if ($secretContentType -ne '') {
        Write-Log "Setting Azure Key Vault $keyVaultName Secret $secretName Attributes"

        az keyvault secret set-attributes -n $secretName --vault-name $keyVaultName --content-type $secretContentType --only-show-errors
        Confirm-LastExitCode
    }
}