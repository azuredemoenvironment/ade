...
resource adeWebVmCustomScriptExtension 'Microsoft.Compute/virtualMachines/extensions@2020-12-01' = [for (adeWebVirtualMachine, i) in adeWebVirtualMachines: {
  ...
  properties: {
    ...
    protectedSettings: {
      ...
      commandToExecute: './${scriptName} "${containerRegistryName}" "${containerRegistryPassword}" "${appConfigConnectionString}" "${adeWebVirtualMachine.adeModule}" "${adeAppVmLoadBalancerPrivateIpAddress}" "${deploymentScriptsBaseUri}/azure_virtual_machines/nginx.conf"'
    }
  }
}]
